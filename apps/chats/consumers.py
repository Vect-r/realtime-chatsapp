...
def get_rendered_message_html(message_id, user):
    ...
    return render_to_string("partials/chat-pills-p.html", context={"msg": message, 'user': user})
...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def status_handler(self, event):
        ...
        if event['user_id'] != self.user.id:
            # status_text = "Online" if event['status'] else f"Last Seen: {timesince.timesince(event['last_seen'], timezone.now())} Ago"
            # # if event['status']:
            # #     status_text = "Online"
            # # elif event['last_seen']:
            # #     status_text = timesince.timesince(event_time, timezone.now())
            # status_class = "text-green-300" if event['status'] else "text-white/50"
            
            # html = f'<p id="user-status" class="text-xs {status_class}">{status_text}</p>'
            # await self.send(text_data=html)
            html = await get_rendered_status_html(event['user_id'])
            await self.send(text_data=html)
